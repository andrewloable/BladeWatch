...
void main() {
  ...
  // ── BladeWatch-9b0f: per-quadrant ROI motion zones ────────────────────────
  //
  // The pipeline has supported these all along (applyQuadrantRoi) and
  // SurveillanceApiHandler already persists them — there was simply no client.

  group('ROI', () {
    test('loads polygons and enabled flags out of GetConfig', () async {
      rpc.stubJson('SurveillanceService', 'GetConfig', {
        'success': true,
        'config': {
          'enabled': true,
      ...
      expect(c.roiPolygonFor('Q0').length, 3);
      expect(c.roiPolygonFor('Q0').first.dx, closeTo(0.1, 0.0001));
      expect(c.roiPolygonFor('Q1'), isEmpty, reason: 'a quadrant with no zone is simply absent');
      expect(c.roiEnabledFor('Q0'), isTrue);
      expect(c.roiEnabledFor('Q1'), isFalse);
      expect(c.roiEnabledFor('Q2'), isTrue);
    });

    test('sends drawn polygons and the enabled flags back on apply', () async {
      stubHappyPath();
      rpc.stubJson('SurveillanceService', 'SetConfig', {'success': true});
      final c = build();
      await c.load();

      c.setRoiPolygon('Q1', const [Offset(0, 0), Offset(1, 0), Offset(1, 1)]);
      c.setRoiEnabled('Q1', true);
      await c.applyChanges(SurveillanceSettingsTab.detection);

      final cfg = (rpc.calls.firstWhere((call) => call.method == 'SetConfig').request as dynamic).config;
      expect(cfg.roiPolygons['Q1'].points.length, 3);
      expect(cfg.roiEnabledQ1, isTrue);
    });

    test('a polygon below the engine minimum is NOT sent', () async {
      // applyQuadrantRoi treats fewer than 3 vertices as "clear this quadrant",
      // so sending a half-drawn shape would silently wipe the saved one.
      stubHappyPath();
      rpc.stubJson('SurveillanceService', 'SetConfig', {'success': true});
      final c = build();
      await c.load();

      c.setRoiPolygon('Q3', const [Offset(0, 0), Offset(1, 1)]);
      await c.applyChanges(SurveillanceSettingsTab.detection);

      final cfg = (rpc.calls.firstWhere((call) => call.method == 'SetConfig').request as dynamic).config;
      expect(cfg.roiPolygons.containsKey('Q3'), isFalse);
    });

    test('undo drops only the last vertex', () async {
      stubHappyPath();
      final c = build();
      await c.load();
      c.setRoiPolygon('Q0', const [Offset(0, 0), Offset(1, 0), Offset(1, 1)]);

      c.undoRoiPoint('Q0');

      expect(c.roiPolygonFor('Q0'), const [Offset(0, 0), Offset(1, 0)]);
    });

    test('undo on an empty zone is a no-op, not a crash', () async {
      stubHappyPath();
      final c = build();
      await c.load();
      ...
      expect(c.roiPolygonFor('Q0'), isEmpty);
    });

    test('clear removes the whole zone', () async {
      stubHappyPath();
      final c = build();
      await c.load();
      c.setRoiPolygon('Q0', const [Offset(0, 0), Offset(1, 0), Offset(1, 1)]);
    ...
    test('disabling a zone keeps the shape the user drew', () async {
      // The daemon models polygon and enabled separately for exactly this
      // reason — switching a zone off must not discard the drawing.
      stubHappyPath();
      final c = build();
      await c.load();
      c.setRoiPolygon('Q0', const [Offset(0, 0), Offset(1, 0), Offset(1, 1)]);
      c.setRoiEnabled('Q0', true);

      c.setRoiEnabled('Q0', false);

      expect(c.roiEnabledFor('Q0'), isFalse);
      expect(c.roiPolygonFor('Q0').length, 3);
    });
  });
  });
