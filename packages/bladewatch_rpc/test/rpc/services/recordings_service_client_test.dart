...
void main() {
  group('RecordingsServiceClient', () {
    ...
    test('markRecording sends RecordingsService/MarkRecording and decodes a real proto3Json response', () async {
      fake.stubJson('RecordingsService', 'MarkRecording', <String, dynamic>{});

      final result = await client.markRecording(MarkRecordingRequest());

      expect(result, isA<MarkRecordingResponse>());
      expect(fake.calls.single.service, 'RecordingsService');
      expect(fake.calls.single.method, 'MarkRecording');
      expect(fake.calls.single.request, isA<MarkRecordingRequest>());
    });
