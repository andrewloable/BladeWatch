...
void main() {
  group('StorageServiceClient', () {
    ...
    test('previewStorageLimitChange sends StorageService/PreviewStorageLimitChange and decodes a real proto3Json response', () async {
      fake.stubJson('StorageService', 'PreviewStorageLimitChange', <String, dynamic>{});

      final result = await client.previewStorageLimitChange(PreviewStorageLimitChangeRequest());

      expect(result, isA<PreviewStorageLimitChangeResponse>());
      expect(fake.calls.single.service, 'StorageService');
      expect(fake.calls.single.method, 'PreviewStorageLimitChange');
      expect(fake.calls.single.request, isA<PreviewStorageLimitChangeRequest>());
    });
