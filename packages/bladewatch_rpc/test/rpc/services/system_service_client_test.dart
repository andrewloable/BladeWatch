...
void main() {
  group('SystemServiceClient', () {
    ...
    test('performanceConnect sends SystemService/PerformanceConnect and decodes a real proto3Json response', () async {
      fake.stubJson('SystemService', 'PerformanceConnect', <String, dynamic>{});

      final result = await client.performanceConnect(PerformanceConnectRequest());

      expect(result, isA<PerformanceConnectResponse>());
      expect(fake.calls.single.service, 'SystemService');
      expect(fake.calls.single.method, 'PerformanceConnect');
      expect(fake.calls.single.request, isA<PerformanceConnectRequest>());
    });

    test('performanceHeartbeat sends SystemService/PerformanceHeartbeat and decodes a real proto3Json response', () async {
      fake.stubJson('SystemService', 'PerformanceHeartbeat', <String, dynamic>{});

      final result = await client.performanceHeartbeat(PerformanceHeartbeatRequest());

      expect(result, isA<PerformanceHeartbeatResponse>());
      expect(fake.calls.single.service, 'SystemService');
      expect(fake.calls.single.method, 'PerformanceHeartbeat');
      expect(fake.calls.single.request, isA<PerformanceHeartbeatRequest>());
    });

    test('performanceDisconnect sends SystemService/PerformanceDisconnect and decodes a real proto3Json response', () async {
      fake.stubJson('SystemService', 'PerformanceDisconnect', <String, dynamic>{});

      final result = await client.performanceDisconnect(PerformanceDisconnectRequest());

      expect(result, isA<PerformanceDisconnectResponse>());
      expect(fake.calls.single.service, 'SystemService');
      expect(fake.calls.single.method, 'PerformanceDisconnect');
      expect(fake.calls.single.request, isA<PerformanceDisconnectRequest>());
    });
