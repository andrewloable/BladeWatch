...
- Update checksum and OTA behavior: [build.gradle.kts:8](../app/build.gradle.kts#L8). The in-app OTA updater and `UpdateApiHandler.java` (removed with the in-app OTA updater in `13e5366`) — this smoke-test item no longer has an implementation to exercise.
