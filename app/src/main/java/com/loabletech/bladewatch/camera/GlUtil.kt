 ...
 * - Error checking
 *
 * An `object` with `@JvmStatic` members so Java callers (and the GPU pipelines) keep spelling
 * `GlUtil.createProgram(...)` unchanged, and Kotlin callers spell it the same way.
 */
object GlUtil {
    ...
    private fun compileShader(shaderType: Int, source: String): Int {
        ...
        if (compileStatus[0] != GLES20.GL_TRUE) {
            ...
            val shaderTypeName = if (shaderType == GLES20.GL_VERTEX_SHADER) "vertex" else "fragment"
    ...
    fun createFloatBuffer(data: FloatArray): FloatBuffer {
        val bb = ByteBuffer.allocateDirect(data.size * 4) // 4 bytes per float
    ...
    fun checkGlError(operation: String) {
        while (true) {
            ...
            val errorMsg = String.format("%s: GL error 0x%x (%d)", operation, error, error)
