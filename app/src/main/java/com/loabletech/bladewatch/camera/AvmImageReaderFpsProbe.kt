 ...
 */
class AvmImageReaderFpsProbe(private val probeDir: File) {
    ...
    fun run() {
        try {
            ...
            logger.error(
                "ImageReader FPS probe top-level failure: " +
    ...
    private fun runUnsafe() {
        if (!probeDir.exists() && !probeDir.mkdirs()) {
            logger.error("Could not create probe dir: " + probeDir.absolutePath)
            return
        }
        logger.info("=== IMAGEREADER FPS PROBE START ===")
        logger.info("Output dir: " + probeDir.absolutePath)
        ...
        //
        // Unqualified because HardwareBufferTextureBinder.kt declares it as a top-level
        // function in THIS package (the form that keeps the JNI symbol unmangled).
        try {
            logger.info(
                "EGL/GL extension probe (no current display): " + probeExtensionsNative()
            )
        } catch (t: Throwable) {
            logger.warn("EGL extension probe failed: " + t.message)
        }
        ...
        val cbThread = HandlerThread("AvmImageReaderProbeCb")
        cbThread.start()
        val cbHandler = Handler(cbThread.looper)

        var cameraObj: Any? = null
        var reader: ImageReader? = null
        var readerSurface: Surface? = null
        val counter = FrameCounter()
        try {
            // ── 1. Open camera (constructor path) ────────────────────────────
            val ctor = avmClass.getDeclaredConstructor(Int::class.javaPrimitiveType)
            ctor.isAccessible = true
            cameraObj = ctor.newInstance(CAMERA_ID)
            val mOpen = avmClass.getDeclaredMethod("open")
            mOpen.isAccessible = true
            val openResult = mOpen.invoke(cameraObj)
            if (openResult !is Boolean || !openResult) {
                logger.error(
                    "AVMCamera.open($CAMERA_ID) returned $openResult — aborting"
                )
                return
            }
            ...
            reader = try {
                ...
                logger.warn(
                    "ImageReader PRIVATE/USAGE_GPU init failed: " +
                        t.message + " — retrying YUV_420_888"
                )
                ImageReader.newInstance(
                    IMAGE_WIDTH, IMAGE_HEIGHT,
                    ImageFormat.YUV_420_888,
                    IMAGE_READER_MAX_IMAGES
                ).also { logger.info("ImageReader created YUV_420_888 fallback") }
            }
            readerSurface = reader.surface
            reader.setOnImageAvailableListener(counter, cbHandler)

            // ── 3. Attach to HAL and start ───────────────────────────────────
            val mAddSurface = avmClass.getDeclaredMethod(
                "addPreviewSurface", Surface::class.java, Int::class.javaPrimitiveType
            )
            mAddSurface.isAccessible = true
            val addRes = mAddSurface.invoke(cameraObj, readerSurface, CAMERA_SURFACE_MODE)
            logger.info("addPreviewSurface(reader, $CAMERA_SURFACE_MODE) → $addRes")

            val mStart = avmClass.getDeclaredMethod("startPreview")
            mStart.isAccessible = true
            val startRes = mStart.invoke(cameraObj)
            logger.info("startPreview → $startRes")

            // ── 4. Wait for frames and measure ───────────────────────────────
            val t0 = System.currentTimeMillis()
            val firstFrameDeadline = t0 + FIRST_FRAME_TIMEOUT_MS
            var stopAt = -1L
            var firstSeen = false
            while (true) {
                ...
                if (!firstSeen && now >= firstFrameDeadline) {
                    logger.warn(
                        "No frames in $FIRST_FRAME_TIMEOUT_MS ms — declaring probe dead"
                    )
                    break
                }
                try {
            ...
            // ── 5. Report ────────────────────────────────────────────────────
            val frames = counter.framesReceived
            var achievedFps = 0f
            if (firstSeen) {
                val dur = maxOf(1L, System.currentTimeMillis() - counter.firstFrameMs)
                achievedFps = frames * 1000f / dur
            }
            logger.info(
                String.format(
                    "── RESULT cam=%d %dx%d framesReceived=%d achievedFps=%.2f " +
                        "addSurface=%s start=%s",
            ...
            if (achievedFps >= 12f) {
                logger.info(
                    "DECISION: ImageReader unblocks the throttle " +
                        "(achievedFps >= 12) — proceed with live-pipeline rewrite"
                )
            } else if (achievedFps > 0) {
                logger.info(
                    "DECISION: ImageReader does NOT unblock the throttle " +
                        "(achievedFps < 12) — the clamp is HAL-side; abort rewrite plan"
                )
            } else {
                logger.warn(
                    "DECISION: probe inconclusive — no frames received. " +
            ...
            if (reader != null) {
                try {
                    ...
                    logger.warn("Failed to close ImageReader: " + ignored.message)
                }
            }
            if (readerSurface != null) {
                try {
                    ...
                    logger.warn("Failed to release reader surface: " + ignored.message)
                }
            }
            if (cameraObj != null) {
     ...
     */
    private class FrameCounter : ImageReader.OnImageAvailableListener {
        @Volatile
        var framesReceived = 0

        @Volatile
        var firstFrameMs: Long = 0

        @Volatile
        private var loggedFirstFrame = false

        @Volatile
        private var loggedHwBufferProbe = false
        ...
        override fun onImageAvailable(r: ImageReader) {
            ...
            try {
                ...
                if (!loggedFirstFrame) {
                    ...
                    logger.info(
                        "first onImageAvailable: format=0x" +
                            Integer.toHexString(image.format) +
                ...
                if (!loggedHwBufferProbe) {
                    ...
                    var hwb: HardwareBuffer? = null
                    try {
                        hwb = image.hardwareBuffer
                        if (hwb != null) {
                            logger.info(
                                "HardwareBuffer extracted ok: format=0x" +
                                    Integer.toHexString(hwb.format) +
                                    " usage=0x" + java.lang.Long.toHexString(hwb.usage) +
                                    " w=" + hwb.width + " h=" + hwb.height
                            )
                        } else {
                            logger.warn("Image.getHardwareBuffer() returned null")
                        }
                    } catch (t: Throwable) {
                        logger.warn("Image.getHardwareBuffer() threw: " + t.message)
                    } finally {
                        if (hwb != null) {
                            try {
                                hwb.close()
                                ...
                                logger.warn(
                                    "Failed to close HardwareBuffer: " + ignored.message
                                )
                            }
                        }
                    }
                ...
                logger.warn("onImageAvailable error: " + t.message)
            } finally {
                if (image != null) {
                    try {
                        image.close()
                    } catch (ignored: Throwable) {
                        logger.warn("Failed to close Image: " + ignored.message)
                    }
                }
            }
        }
    ...
    companion object {
        private val logger = DaemonLogger.getInstance("AvmImageReaderFpsProbe")

        /** Camera id matching the live panoramic pipeline. */
        private const val CAMERA_ID = 1
        private const val CAMERA_SURFACE_MODE = 0
        private const val IMAGE_WIDTH = 5120
        ...
        private const val RECORD_MS = 30_000L
        private const val FIRST_FRAME_TIMEOUT_MS = 5_000L

        private fun tryInvoke(cls: Class<*>, instance: Any, name: String) {
            try {
                val m = cls.getDeclaredMethod(name)
                m.isAccessible = true
                m.invoke(instance)
            } catch (ignored: Throwable) {
                logger.warn("tryInvoke $name failed: " + ignored.message)
            }
        }
    }
}
