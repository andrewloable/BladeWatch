...
class DetectionBaseline {
     ...
     * [CONFIRMED_HIT_COUNT] times before [isInBaseline] treats
     * it as authoritative).
     */
    class Entry(
        @JvmField val classId: Int,
        // Normalized to quadrant dimensions
        ...
        @JvmField
        var addedAtMs: Long = System.currentTimeMillis()

        @JvmField
        var lastSeenMs: Long = addedAtMs

        @JvmField
        var hitCount: Int = 1

        fun isConfirmed(): Boolean = hitCount >= CONFIRMED_HIT_COUNT

        /** Bottom-centre Y in normalized coords — the foot-point anchor. */
        fun footY(): Float = cy + h / 2.0f
    ...
    private val baselines = Array(NUM_QUADRANTS) { ArrayList<Entry>() }

    /**
     * Per-quadrant recent person detections (for spatial veto).
     * Stores person detections from the last 60 seconds of each event.
     */
    private val recentPersons = Array(NUM_QUADRANTS) { ArrayList<PersonRecord>() }
    ...
    fun seedFromDetections(
        ...
        for (det in detections) {
            if (det.confidence < MIN_BASELINE_CONFIDENCE) continue
            if (isNeverPromoteClass(det.classId)) continue

            val cx = (det.x + det.w / 2.0f) / quadW
            val cy = (det.y + det.h / 2.0f) / quadH
            val w = det.w.toFloat() / quadW
            val h = det.h.toFloat() / quadH
            ...
            val canonical = canonicalClass(det.classId)
            var duplicate = false
            for (existing in baselines[quadrant]) {
                if (canonicalClass(existing.classId) != canonical) continue
                val dx = existing.cx - cx
                val dy = existing.cy - cy
                if (sqrt((dx * dx + dy * dy).toDouble()) > FOOTPOINT_MATCH_DIST_NORM) continue
                val existingArea = existing.w * existing.h
                val detArea = w * h
                val areaRatio = if (existingArea > 0 && detArea > 0) {
                    max(detArea / existingArea, existingArea / detArea)
                } else {
                    99f
                }
            ...
            val e = Entry(det.classId, cx, cy, w, h, quadrant)
            e.hitCount = CONFIRMED_HIT_COUNT
            baselines[quadrant].add(e)
        }

        logger.info(
            "Baseline seeded for Q" + quadrant + ": " + baselines[quadrant].size +
                " entries (confirmed)"
        )
    }
     ...
     */
    @Synchronized
    fun isInBaseline(det: Detection, quadrant: Int, quadW: Int, quadH: Int): Boolean {
        if (quadrant < 0 || quadrant >= NUM_QUADRANTS) return false

        val detCx = (det.x + det.w / 2.0f) / quadW
        val detCy = (det.y + det.h / 2.0f) / quadH
        val detW = det.w.toFloat() / quadW
        val detH = det.h.toFloat() / quadH
        val detFootX = detCx
        val detFootY = detCy + detH / 2.0f
        ...
        val occludedByPerson = anyRecentPersonOverlaps(quadrant, detCx, detCy, detW, detH)
        val footDistTolerance = if (occludedByPerson) {
        ...
        val iter = baselines[quadrant].iterator()
        while (iter.hasNext()) {
            val entry = iter.next()
            ...
            if (canonicalClass(entry.classId) != canonicalClass(det.classId)) continue

            // Path 1: bbox IoU match (the original strict check).
            val iou = computeIoU(detCx, detCy, detW, detH, entry.cx, entry.cy, entry.w, entry.h)
            ...
            val dx = detFootX - entry.footX()
            val dy = detFootY - entry.footY()
            val footDist = sqrt((dx * dx + dy * dy).toDouble()).toFloat()
            if (footDist <= footDistTolerance) {
                if (occludedByPerson) {
                    return true
                }
                val widthRatio = if (entry.w > 0) max(detW / entry.w, entry.w / detW) else 99f
                val heightRatio = if (entry.h > 0) max(detH / entry.h, entry.h / detH) else 99f
                // Area-ratio discriminator (FIX A10) — see matchAndRefresh.
                val entryArea = entry.w * entry.h
                val areaRatio = if (entryArea > 0) {
                    max((detW * detH) / entryArea, entryArea / (detW * detH))
                } else {
                    99f
                }
    ...
    private fun anyRecentPersonOverlaps(
        ...
        val stalenessCutoff = System.currentTimeMillis() - 2000
        for (p in recentPersons[quadrant]) {
     ...
     */
    @Synchronized
    fun recordPersonDetection(quadrant: Int, det: Detection, quadW: Int, quadH: Int) {
        if (quadrant < 0 || quadrant >= NUM_QUADRANTS) return
        if (det.classId != 0) return // Only person (class 0)

        recentPersons[quadrant].add(
            PersonRecord(
                (det.x + det.w / 2.0f) / quadW,
                (det.y + det.h / 2.0f) / quadH,
                det.w.toFloat() / quadW,
                det.h.toFloat() / quadH
            )
        )
        ...
        recentPersons[quadrant].removeIf { it.timestampMs < cutoff }
    }
    ...
    fun updateFromEventEnd(
        ...
        for (det in detections) {
            ...
            if (isNeverPromoteClass(det.classId)) {
                skippedLiving++
                continue
            }
            ...
            if (matchAndRefresh(quadrant, det.classId, cx, cy, w, h)) {
                refreshed++
            } else {
                baselines[quadrant].add(Entry(det.classId, cx, cy, w, h, quadrant))
                added++
            }
        }
        ...
        if (added > 0 || refreshed > 0 || vetoed > 0) {
            logger.info(
                "Baseline update Q" + quadrant + ": +" + added + " new, " + refreshed +
    ...
    fun promoteStaticActor(
        ...
        val cx = (x + bw / 2.0f) / quadW
        val cy = (y + bh / 2.0f) / quadH
        val w = bw.toFloat() / quadW
        val h = bh.toFloat() / quadH
        ...
        if (!matchAndRefresh(quadrant, classId, cx, cy, w, h)) {
            baselines[quadrant].add(Entry(classId, cx, cy, w, h, quadrant))
        }
    }
    ...
    private fun matchAndRefresh(
        ...
        val footY = cy + h / 2.0f
        val now = System.currentTimeMillis()
        val canonicalIncoming = canonicalClass(classId)
        for (entry in baselines[quadrant]) {
            if (canonicalClass(entry.classId) != canonicalIncoming) continue
            var isMatch = false
            // IoU path
            val iou = computeIoU(cx, cy, w, h, entry.cx, entry.cy, entry.w, entry.h)
            if (iou >= MATCH_IOU_THRESHOLD) {
                isMatch = true
            } else {
                // Foot-point path
                val dx = footX - entry.footX()
                val dy = footY - entry.footY()
                if (sqrt((dx * dx + dy * dy).toDouble()) <= FOOTPOINT_MATCH_DIST_NORM) {
                    val wRatio = if (entry.w > 0) max(w / entry.w, entry.w / w) else 99f
                    val hRatio = if (entry.h > 0) max(h / entry.h, entry.h / h) else 99f
                    // Additional area-ratio check (FIX A10) — discriminates
                    // same-physical-vehicle from adjacent-vehicle merge.
                    val entryArea = entry.w * entry.h
                    val areaRatio = if (entryArea > 0) {
                        max((w * h) / entryArea, entryArea / (w * h))
                    } else {
                        99f
                    }
            ...
            if (isMatch) {
                ...
                if (entry.hitCount < Int.MAX_VALUE - 1) entry.hitCount++
                return true
            }
        }
        return false
     ...
     */
    @Synchronized
    fun refreshQuadrant(quadrant: Int, detections: List<Detection>?, quadW: Int, quadH: Int) {
        if (quadrant < 0 || quadrant >= NUM_QUADRANTS) return
        ...
        if (detections.isNullOrEmpty()) {
            ...
            logger.info(
                "Baseline refresh Q" + quadrant + ": YOLO returned empty, keeping " +
        ...
        logger.info(
            "Baseline refresh Q" + quadrant + " (lighting transition): +" + added +
    ...
    @Synchronized
    fun getEntryCount(quadrant: Int): Int {
    ...
    private fun overlapsRecentPerson(
        quadrant: Int,
        cx: Float,
        cy: Float,
        w: Float,
        h: Float
    ): Boolean = recentPersons[quadrant].any {
        computeIoU(cx, cy, w, h, it.cx, it.cy, it.w, it.h) >= SPATIAL_VETO_IOU_THRESHOLD
    }

    companion object {
        private val logger = DaemonLogger.getInstance("DetectionBaseline")

        private const val NUM_QUADRANTS = 4

        /** IoU threshold for matching a detection against a baseline entry */
        private const val MATCH_IOU_THRESHOLD = 0.7f

        /** IoU threshold for spatial veto (overlap with recent person detections) */
        private const val SPATIAL_VETO_IOU_THRESHOLD = 0.3f

        /** Minimum confidence to add to baseline */
        private const val MIN_BASELINE_CONFIDENCE = 0.4f
         ...
         */
        private const val FOOTPOINT_MATCH_DIST_NORM = 0.04f
         ...
         */
        private const val FOOTPOINT_AREA_RATIO_MAX = 1.15f
         ...
         */
        private const val CONFIRMED_HIT_COUNT = 3
         ...
         *
         * We measure from `lastSeenMs`, not `addedAtMs`. A continuously
         ...
         */
        private const val BASELINE_ENTRY_MAX_AGE_MS = 2 * 60 * 60 * 1000L // 2 hours

        /**
         * COCO class IDs that are NEVER promotable (living things).
         * 0=person, 15=cat, 16=dog, 14=bird.
         */
        private val NEVER_PROMOTE_CLASSES = intArrayOf(0, 14, 15, 16)

        private fun isNeverPromoteClass(classId: Int): Boolean =
            NEVER_PROMOTE_CLASSES.any { it == classId }
         ...
         */
        private fun canonicalClass(classId: Int): Int = when (classId) {
            5, 7 -> 2 // bus, truck → car
            3 -> 1 // motorcycle → bicycle
            else -> classId
        }
        ...
        private fun computeIoU(
            ...
            val right1 = cx1 + w1 / 2
            val top1 = cy1 - h1 / 2
            val bottom1 = cy1 + h1 / 2
            val left2 = cx2 - w2 / 2
            val right2 = cx2 + w2 / 2
            val top2 = cy2 - h2 / 2
            val bottom2 = cy2 + h2 / 2

            val interLeft = max(left1, left2)
            val interTop = max(top1, top2)
            val interRight = min(right1, right2)
            val interBottom = min(bottom1, bottom2)
