 ...
 * [MotionPipelineV2.QUADRANT_NAMES], which is {"front", "right", "rear", "left"}:
 *
...
class FoveatedCropper {
    ...
    fun init() {
        try {
            ...
            rgbBuffer = ByteArray(CROP_SIZE * CROP_SIZE * 3)
            ...
            logger.error("FoveatedCropper init failed: " + e.message)
        }
    }
     ...
     */
    fun crop(cameraTextureId: Int, quadrant: Int, centroidX: Float, centroidY: Float): ByteArray? {
        if (!initialized || quadrant < 0 || quadrant >= 4) return null
        val read = readBuffer ?: return null
        val rgb = rgbBuffer ?: return null
        ...
        val quadPixelX = (centroidX + 0.5f) * 32.0f // Center of block, in 320px space
        val quadPixelY = (centroidY + 0.5f) * 32.0f // Center of block, in 240px space
        ...
        val camNormX = quadPixelX / 320.0f // [0, 1] within camera frame
        val camNormY = quadPixelY / 240.0f // [0, 1] within camera frame

        // Step 3: Compute the crop window in the full 5120×960 strip.
        // Each camera occupies 0.25 of the strip width.
        val stripOffsetX = QUADRANT_STRIP_OFFSET_X[quadrant]
        ...
        val cropWidthNorm = CROP_SIZE.toFloat() / STRIP_WIDTH // 0.125
        val cropHeightNorm = CROP_SIZE.toFloat() / STRIP_HEIGHT // 0.6667
        ...
        val centerX = stripOffsetX + camNormX * 0.25f
        val centerY = camNormY

        // Compute crop rect, clamped to the camera's strip region
        // (don't bleed into adjacent cameras)
        val camRight = stripOffsetX + 0.25f
        val cropLeft = maxOf(
            stripOffsetX,
            minOf(centerX - cropWidthNorm / 2.0f, camRight - cropWidthNorm)
        )
        val cropTop = maxOf(
            0.0f,
            minOf(centerY - cropHeightNorm / 2.0f, 1.0f - cropHeightNorm)
        )
        ...
        read.clear()
        GLES20.glReadPixels(
            0, 0, CROP_SIZE, CROP_SIZE, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, read
        )
        ...
        read.rewind()
        var rgbIdx = 0
        for (y in CROP_SIZE - 1 downTo 0) {
            val rowStart = y * CROP_SIZE * 4
            for (x in 0 until CROP_SIZE) {
                val srcIdx = rowStart + x * 4
                rgb[rgbIdx++] = read.get(srcIdx)
                rgb[rgbIdx++] = read.get(srcIdx + 1)
                rgb[rgbIdx++] = read.get(srcIdx + 2)
            }
        }
    ...
    companion object {
        private val logger = DaemonLogger.getInstance("FoveatedCrop")

        /** Output crop size — matches YOLO input */
        const val CROP_SIZE = 640

        // Source strip dimensions. Each camera occupies 1/4 of the strip = 1280×960.
         ...
         *
         * From the mosaic fragment shader (GpuDownscaler):
         ...
         */
        private val QUADRANT_STRIP_OFFSET_X = floatArrayOf(
            0.75f, // Q0 (TL → Front)
            0.50f, // Q1 (TR → Right)
            0.00f, // Q2 (BL → Rear)
            0.25f // Q3 (BR → Left)
